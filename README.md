# Firewall-Conn-Log-LLM-Analysis

🛡️ Cisco ASA Connection Log Analyzer
...
Source Interface Name (e.g., inside)
...
Translated Source IP (NAT/XLATE)
...
Destination Interface Name (e.g., outside)
...
Translated Destination IP (NAT/XLATE)
...
Time since last activity (e.g., 0:02:15)

uptime

TEXT

Total connection duration (e.g., 1h5m2s)

bytes_transferred

INTEGER

Total bytes for the connection

flags

TEXT

Connection flags (e.g., UR, N, P)

initiator_ip

TEXT

IP that initiated the connection (Format 2 only)

responder_ip

TEXT

IP that responded to the connection (Format 2 only)
